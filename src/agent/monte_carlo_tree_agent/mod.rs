...
impl MonteCarloTreeAgent {
	...
	fn tree_root(&self, board: UltimateBoard) -> Option<u8> {
		...
		let best_child = root.borrow().get_children().iter().map(|child| {
			let child_stats = child.borrow().get_data().stats;
			let uct = child_stats.wins() as f64 / child_stats.total() as f64;
			(child, uct)
		}).max_by(|a, b| a.1.partial_cmp(&b.1).unwrap_or(std::cmp::Ordering::Equal)).map(|(child, _)| child).unwrap().clone();
	...
	fn playout(&self, board: UltimateBoard) -> Stats {
		...
		while board.get_game_status() == GameResult::Continue {
			...
			let next_move = possible_moves[Uniform::from(0..possible_moves.len()).sample(&mut rand::thread_rng())];
...
impl Agent for MonteCarloTreeAgent {
	...
	fn get_info(&self) -> AgentInfo {
		AgentInfo::new("MonteCarloTree".to_string(), self.player, self.turn, format!("max_nodes: {}", self.iterations))
	}
}
